...
export interface ToastItem {
  ...
  tone: ToastTone
  text: string
  /** Duracao propria; ausente usa `TOAST_TTL_MS`. */
  ttlMs?: number
}
...
/**
 * Duracao para aviso que precisa ser LIDO, nao so notado: quatro segundos e o
 * tempo de NOTAR um aviso de status; doze e o de LER duas frases enquanto se
 * comeca a transmitir.
 */
export const TOAST_TTL_LONG_MS = 12_000
...
export interface AppStore {
  ...
  pushToast: (tone: ToastTone, text: string, ttlMs?: number) => void
...
export const useAppStore = create<AppStore>((set) => ({
  ...
  pushToast: (tone, text, ttlMs) =>
    set((current) => {
      toastSeq += 1
      // Teto defensivo: uma rajada de eventos nunca vira uma pilha infinita.
      const toasts = [...current.toasts, { id: toastSeq, tone, text, ttlMs }]
