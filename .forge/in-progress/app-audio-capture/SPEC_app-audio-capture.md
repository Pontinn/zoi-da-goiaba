...
- O addon roda dentro de um **`utilityProcess`** do Electron (isolamento de crash: um bug no C++ derruba o worker, nao o app; `utilityProcess.fork` existe no Electron 43, `electron.d.ts` linha 15688/15696, e carrega modulos nativos - confirmado pelo spike: o probe WASAPI deu ok DENTRO do utilityProcess).
- **PERIGO MEDIDO no spike (SPIKE-RESULTS secao 8)**: uma variavel `ELECTRON_RUN_AS_NODE` herdada no ambiente, MESMO VAZIA, mata o `utilityProcess` com `Assertion failed: (isolate_data->snapshot_data()) != nullptr`. O fork do worker DEVE limpar essa variavel (passar `env` explicito sem `ELECTRON_RUN_AS_NODE` nas opcoes do `utilityProcess.fork`). Registro relacionado para scripts futuros: o Electron 43 nao roda um entry `.mjs` como processo main (sobe a janela do app padrao e trava); scripts de diagnostico devem usar bootstrap CJS (o app real nao e afetado: o electron-vite ja emite CJS para o main).
...
3. **`restrictOwnAudio` do Chromium 141+** como solucao: filtra so o audio do proprio documento capturante; nao cobre o Discord, entao nao substitui o addon. **CONFIRMADO pelo spike (SPIKE-RESULTS secao 5), mais forte do que se supunha**: `getSupportedConstraints().restrictOwnAudio === true`, `getDisplayMedia({ audio: { restrictOwnAudio: true } })` resolve por cima do handler `'loopback'` do Electron e `track.getSettings().restrictOwnAudio === true` (constraint APLICADO, nao ignorado). Decisao: segue fora do caminho principal (o addon ja exclui o Zoi por processo), mas o Sprint 4 PODE adotar o constraint no caminho DEGRADADO (`full-loopback`), onde ele e uma defesa extra de graca contra o proprio audio do app; fica como opt-in do agente do Sprint 4, sem virar dependencia da arquitetura.
...
```ts
// Frame PCM de 10 ms. `data` e um ArrayBuffer COPIADO por structured clone
// (postMessage SEM lista de transferencia), Float32 interleaved LR, 48000 Hz,
// 2 canais, 480 amostras por canal. Custo medido: ~384 KB/s, desprezivel.
{ type: 'pcm'; timestampUs: number; data: ArrayBuffer }
```

**AVISO (fato medido no spike, SPIKE-RESULTS secao 4.2)**: o `MessagePortMain` do Electron aceita SOMENTE instancias de `MessagePortMain` na lista de transferencia; tentar transferir o ArrayBuffer (`port.postMessage(msg, [arrayBuffer])`) lanca `Port at index 0 is not a valid port` e MATA o fluxo de audio a cada frame. O worker DEVE enviar sem lista de transferencia (copia via structured clone); nao existe caminho zero-copy aqui e ele nao e necessario (RNF-01 preservado com folga).
...
- **Toolchain**: `node-gyp` (script `install` do pacote local), MSVC Build Tools + Python 3.12 na maquina de build (fato do spike: Python instalado via winget, secao 9); ABI estavel por Node-API (sem electron-rebuild por versao). Script novo no package.json raiz: `"postinstall": "electron-builder install-app-deps"` (padrao electron-builder para deps nativas). ATENCAO npm 11: entrada `"zoi-audio-capture@0.1.0": true` obrigatoria em `allowScripts`, senao o rebuild do pacote e pulado em silencio (fato do spike, secao 9).
...
- **Feature 3.1** `[category: ipc]` Orquestracao main + worker + port
  ...
  - **Steps**:
    1. `audio-exclusion.ts`: `startAudioExclusion(getWindow): Promise<AudioExclusionStartResult>`: (a) se `process.env['ZOI_DISABLE_AUDIO_EXCLUSION']` -> `{ mode: 'unavailable', reason: 'disabled-by-env' }`; (b) checar build do SO (parse de `os.release()`, exigir >= 10.0.20348) -> `os-unsupported`; (c) `require('zoi-audio-capture')` e `probe()` -> `addon-load-failed` / `activation-failed`; (d) `utilityProcess.fork(join(__dirname, 'audio-capture-worker.js'), [], { env: envSemElectronRunAsNode })` -> `worker-spawn-failed` (OBRIGATORIO limpar `ELECTRON_RUN_AS_NODE` do env do fork, mesmo vazia ela mata o worker; secao 2.2); (e) `MessageChannelMain`: port1 vai no `postMessage({ type: 'start', config })` ao worker (config com `mode: 'process-exclusion'` neste caminho feliz), port2 vai ao renderer via `getWindow().webContents.postMessage(IPC.audioExclusionPort, { channel: 'zoi:audio-exclusion-port' }, [port2])`; (f) devolver `{ mode: 'process-exclusion', sampleRate: 48000, channels: 2 }`. Config montada 100% no main: `mode` decidido pela cascata, `excludedExecutables` fixo, `excludedRootPids: [process.pid]`.
...
- Nota de execucao: **o Sprint 1 (spike) FOI EXECUTADO** e a evidencia completa vive em `SPIKE-RESULTS_app-audio-capture.md` neste mesmo diretorio: veredito P2 confirmado empiricamente (o Electron recusa qualquer filtro de processo no loopback), probe WASAPI ok no main E no utilityProcess, `MediaStreamTrackGenerator` funcional, transporte de port validado, `npm run dist` integro com o addon empacotado. Os itens abaixo refletem esse estado.
- `[FATO - spike]` Toolchain da maquina de dev CONFIRMADA: VS Community 2026 (MSVC 14.51) + Windows SDK 10.0.26100 ja presentes; **Python 3.12.10 foi INSTALADO durante o spike** (`winget install Python.Python.3.12 --scope user`) porque o node-gyp exige Python real (o alias da Microsoft Store nao serve). Pre-requisito de maquina de dev documentado.
- `[FATO - spike]` **npm 11 PULA SILENCIOSAMENTE o `node-gyp rebuild` do pacote nativo** sem a entrada `"zoi-audio-capture@0.1.0": true` em `allowScripts` do package.json raiz (a entrada ja foi adicionada no Sprint 1; se a versao do pacote mudar, a chave precisa acompanhar). O stub JS no `index.js` continua como rede de seguranca se o binario faltar.
- `[FATO - spike]` `MediaStreamTrackGenerator` de audio funciona no renderer do Electron 43 (construiu, recebeu `AudioData` de 480 amostras, `readyState: 'live'` dentro de um `MediaStream`); a aceitacao fim-a-fim pelo RTCPeerConnection do PeerJS e validada no smoke do Sprint 4; plano B segue documentado (AudioWorklet + `createMediaStreamDestination`, secao 3 item 5).
- `[FATO - spike]` `restrictOwnAudio` e aceito e APLICADO pelo Chromium 150 do Electron mesmo no caminho `setDisplayMediaRequestHandler` + `'loopback'` (`getSettings().restrictOwnAudio === true`). Nao remove o Discord; fica como opt-in do Sprint 4 para o caminho degradado (secao 3 item 3).
...
Revisao 2 (correcoes dirigidas pelos resultados REAIS do spike do Sprint 1, evidencia em `SPIKE-RESULTS_app-audio-capture.md`): (1) secao 5.C corrigida: o frame PCM viaja por COPIA (structured clone, ~384 KB/s medidos), nunca por lista de transferencia; aviso explicito de que transferir o ArrayBuffer lanca `Port at index 0 is not a valid port` e mata o audio a cada frame; (2) secao 9 atualizada de assumption para FATO: Python 3.12.10 instalado via winget (exigencia do node-gyp) e entrada obrigatoria `"zoi-audio-capture@0.1.0": true` em `allowScripts` do npm 11 (sem ela o rebuild nativo e pulado em silencio), espelhado na secao 5b; (3) secao 3 item 3 atualizado para CONFIRMADO: `restrictOwnAudio` e aceito e aplicado no caminho `'loopback'` do Electron, opt-in do Sprint 4 para o caminho degradado (espelhado na secao 9); (4) secao 2.2 ganhou o perigo `ELECTRON_RUN_AS_NODE` (mesmo vazia mata o utilityProcess; fork do Sprint 3 agora limpa o env explicitamente) e a nota de que Electron 43 nao roda entry `.mjs` no main. Nota de execucao do Sprint 1 adicionada no topo da secao 9. Sem renumeracao de sprints ou requisitos; matriz de cobertura inalterada e sem orfaos.
