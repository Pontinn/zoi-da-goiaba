...
| 2026-08-24 | 5 | Correcoes apontadas pelo reviewer da SPEC (itens upstream na PRD): RF-04/AC-04 restringidos para que "codigo ja em uso" bloqueie apenas a CRIACAO de sala com codigo repetido, nao a entrada em sala existente (que segue o fluxo normal, RF-06/AC-29); AC-23 detalhado para deixar explicito que, em desconexao/banimento, o ALVO ouve o som "desconectado" e os DEMAIS participantes ouvem o som "saiu"; RNF-09 atualizado para aceitar `.m4a` (AAC) entre os formatos de audio embutidos, refletindo o formato real das gravacoes do usuario. |
...
- **RF-04** [MUST] Se o codigo gerado ou digitado para CRIAR uma nova sala ja estiver em uso por outra sala ativa, o sistema deve exibir a mensagem "codigo ja em uso" e impedir a criacao dessa sala. Essa checagem se aplica apenas a criacao: entrar em uma sala ja existente usando o codigo dela e o fluxo normal de entrada (RF-06), sem essa mensagem.
...
- **RNF-09** [MUST] Os sons de notificacao devem ser arquivos de audio (mp3/wav/ogg/m4a-AAC) embutidos no pacote do app (pasta `audios/`), sem dependencia de download externo em tempo de execucao. As gravacoes originais do usuario estao em `.m4a` (AAC), formato aceito e processado normalmente pelo app.
...
- **AC-04** (RF-03, RF-04): Dado um codigo ja em uso por outra sala ativa, quando um usuario tenta CRIAR uma nova sala com esse mesmo codigo, entao o sistema exibe "codigo ja em uso" e a criacao e bloqueada. Entrar (nao criar) em uma sala existente usando esse mesmo codigo continua sendo o fluxo normal de entrada (RF-06, AC-29), sem essa mensagem de bloqueio.
...
- **AC-23** (RF-39): Dado um participante entrando na sala, quando a entrada e concluida, entao os demais participantes ouvem o som "entrou" (e de forma analoga para transmitindo, parou-transmissao, erro-conexao e reconectado, cada um no respectivo evento). Para desconexao/banimento pelo dono especificamente: o ALVO da acao (quem foi desconectado ou banido) ouve o som "desconectado"; os DEMAIS participantes, que continuam na sala, ouvem o som "saiu" (da perspectiva deles, aquela pessoa apenas saiu da sala).
